...
Source code for distributed implementation of Project DisCo using DBSCAN.
Includes DiscoReconstructor class and all support functions.
'''
...
def dist_from_data(X, Y, Nx, Ny):#, row_labels=False, column_labels=False):
    ...
    Nx: int
        Number of unique possible values for X.
    
    Ny: int
        Number of unique possible values for Y.
...
#         print('done: past cluster assignments', flush=True)
        ...
        # for dbscan pipleline, which assumes noise
        self.pasts += 1 
        self._N_pasts += 1
...
#         self.futures = future_local.assignments.reshape(-1)
        ...
        # for dbscan pipeline, which assumes noise
        self.futures += 1 
        self._N_futures += 1
...
#         print('done: flatten the future assignments', flush=True)
    ...
    def dbscan_lightcones(self, past_params, future_params,
                            past_decay=0, future_decay=0):
        '''
        Performs clustering on the distributed arrays of both past and future lightcones.
        ...
        ----------
        past_params: dict,
            Dictionary of keword arguments for past lightcone clustering algorithm.e
...
#         self._N_futures = total number of dbscan labels (i.e. number of clusters + 1 for noise)        
    ...
    def reconstruct_states(self, metric, *metric_args, pval_threshold=0.05, **metric_kwargs):
        ...
        morphs = self.global_joint_dist[1:] # Skip past=0, which is noise state

        # noise past is label 0 and gets mapped to noise state, which is index 0
        ...
        for item in morphs:
            past = item[0]
            morph = item[2:] # skip future=0; it is the noise label
